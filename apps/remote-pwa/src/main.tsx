...
function App() {
  ...
  const createPairingOffer = (): void => { const pairingClient = relayUrl !== '' && relayChannel !== '' && relayToken !== '' && relayKey !== '' ? client : directClient; void pairingClient.pairingOffer().then(offer => { setPairingOffer(offer); setPairCode(offer.code) }).catch(value => setError(value instanceof Error ? value.message : String(value))) }
