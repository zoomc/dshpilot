...
export class GitPresentation {
  ...
  async summary(cwd: string): Promise<{ branch: string; status: string; diff: string }> { const worktree = await this.cwd(cwd); return { branch: (await git(worktree, ['branch', '--show-current'])).trim(), status: await git(worktree, ['status', '--short']), diff: await git(worktree, ['diff', '--no-ext-diff']) } }
}
